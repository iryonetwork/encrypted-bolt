...
func TestOpen_ErrPathRequired(t *testing.T) {
	_, err := bolt.Open(testKey, "", 0666, nil)
	if err == nil {
...
func TestOpen_ErrNotExists(t *testing.T) {
	_, err := bolt.Open(testKey, filepath.Join(tempfile(), "bad-path"), 0666, nil)
	if err == nil {
...
func TestOpen_ErrInvalid(t *testing.T) {
	...
	if _, err := bolt.Open(testKey, path, 0666, nil); err != bolt.ErrInvalid {
...
func TestOpen_ErrChecksum(t *testing.T) {
	...
	// Reopen data file.
	if _, err := bolt.Open(testKey, path, 0666, nil); err != bolt.ErrChecksum {
		t.Fatalf("unexpected error: %s", err)
	}
}
...
func TestOpen_Size(t *testing.T) {
	...
	db0, err := bolt.Open(testKey, path, 0666, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDB_Open_InitialMmapSize(t *testing.T) {
	...
	db, err := bolt.Open(testKey, path, 0666, &bolt.Options{InitialMmapSize: initMmapSize})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDB_Open_ReadOnly(t *testing.T) {
	...
	readOnlyDB, err := bolt.Open(testKey, f, 0666, o)
	if err != nil {
		panic(err)
	}
...
func MustOpenWithOption(o *bolt.Options) *DB {
	f := tempfile()
	db, err := bolt.Open(testKey, f, 0666, o)
	if err != nil {
		panic(err)
	}
...
func (db *DB) MustReopen() {
	indb, err := bolt.Open(testKey, db.f, 0666, db.o)
	if err != nil {
		panic(err)
	}
	db.DB = indb
